...
public sealed class ViewModelGenerator : IIncrementalGenerator
{
    ...
    private static string? GenerateViewModel(INamedTypeSymbol symbol)
    {
        ...
        foreach (var member in symbol.GetMembers())
        {
            ...
            {
                if (IsInterface(p.Type, "IListState", "Mvux.Wpf.Core"))
                    props.Add((p.Name, PropKind.ListState, TypeArg(p.Type)));
                else if (IsInterface(p.Type, "IListFeed", "Mvux.Wpf.Core"))
                    props.Add((p.Name, PropKind.ListFeed, TypeArg(p.Type)));
                else if (IsInterface(p.Type, "IState", "Mvux.Wpf.Core"))
                    props.Add((p.Name, PropKind.State, TypeArg(p.Type)));
                else if (IsInterface(p.Type, "IFeed", "Mvux.Wpf.Core"))
        ...
        sb.AppendLine("using Mvux.Wpf.Core;");
        sb.AppendLine();
        ...
        foreach (var (name, kind, typeArg) in props)
        {
            ...
            switch (kind)
            {
                ...
                case PropKind.Feed:
                    ...
                    sb.AppendLine($"    public global::Mvux.Wpf.Core.IFeed<{typeArg}> {name} => _model.{name};");
                    break;
