...
{
    public static object? ReadSelection(SelectingItemsControl selector, ISelectionFeed feed)
    {
    ...
    public static void ApplySelection(SelectingItemsControl selector, ISelectionFeed feed, object? selection)
    {
        if (feed.SupportsMultipleSelection && TryGetSelectedItems(selector, out var selectedItems))
        {
    ...
    private static bool TryGetSelectedItems(SelectingItemsControl selector, out IList selectedItems)
    {
        selectedItems = null!;
